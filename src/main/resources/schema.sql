...
(
    ...
    (100) NOT NULL,
    role ENUM
    ('CASHIER', 'MANAGER', 'ADMIN', 'ONLINE_CUSTOMER') NOT NULL,
    ...
    total_amount DECIMAL
                                (10,2) NOT NULL,
    payment_method ENUM
                                ('CASH', 'CREDIT_CARD', 'PAYPAL') NOT NULL,
    ...
    FOREIGN KEY (item_id)
                                    ...
                                    -- Add some test stock batches for demonstration
                                    INSERT INTO stock_batch
                                        (item_id, quantity_received, quantity_remaining, purchase_date, expiry_date)
                                    SELECT item_id, 50, 50, '2025-10-01', '2025-11-24'
                                    FROM item
                                    WHERE code = 'RICE001';

                                    INSERT INTO stock_batch
                                        (item_id, quantity_received, quantity_remaining, purchase_date, expiry_date)
                                    SELECT item_id, 30, 30, '2025-10-01', '2025-11-20'
                                    FROM item
                                    WHERE code = 'RICE001';

                                    INSERT INTO shelf_stock
                                        (item_id, quantity)
                                    SELECT item_id, 100
                                    FROM item;
